...
namespace KendoCRUDService.Data.Repositories
{
    public interface IUserDataCache
    {
        ...
        void SetList<T>(string userKey, string logicalName, List<T> list, TimeSpan ttl, bool sliding);
        Dictionary<string, byte[]> GetOrCreateDictionary(string userKey, string logicalName, Func<Dictionary<string, byte[]>> dict, TimeSpan ttl, bool sliding);
        void SetDictionary(string userKey, string logicalName, Dictionary<string, byte[]> files, TimeSpan ttl, bool sliding);
    }
    ...
    public class UserDataCache : IUserDataCache
    {
        ...
        public void SetList<T>(string userKey, string logicalName, List<T> list, TimeSpan ttl, bool sliding)
        {
            var key = BuildKey(userKey, logicalName);
        ...
        public void SetDictionary(string userKey, string logicalName, Dictionary<string, byte[]> dict, TimeSpan ttl, bool sliding)
        {
            var key = BuildKey(userKey, logicalName);
            var opts = new MemoryCacheEntryOptions();
            if (sliding)
            ...
            _cache.Set(key, dict, opts);
        }

        public Dictionary<string, byte[]> GetOrCreateDictionary(string userKey, string logicalName, Func<Dictionary<string, byte[]>> dict, TimeSpan ttl, bool sliding)
        {
            var key = BuildKey(userKey, logicalName);
            if (_cache.TryGetValue(key, out Dictionary<string, byte[]> _dict))
            {
                return _dict;
            }
