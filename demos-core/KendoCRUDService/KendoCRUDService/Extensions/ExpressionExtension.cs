...
namespace KendoCRUDService.Extensions
{
    public static class ExpressionExtension
    {
        public static Expression<Func<T, bool>> GenerateFilterExpression<T>(Filter filter)
        {
            ...
            {
                var currentExpression = filterDesc.IsDescriptor
                    ? CreateOperatorExpression<T>(filterDesc.Operator, filterDesc.Field, filterDesc.Value)
                    : GenerateFilterExpression<T>(filterDesc);

                result = CombineExpressions(filter.Logic, result, currentExpression);
            }

            return result;
        }

        private static Expression<Func<T, bool>> CombineExpressions<T>(string logic, Expression<Func<T, bool>> expr1, Expression<Func<T, bool>> expr2)
        {
            if (expr1 == null) return expr2;

            return logic == "and" ? And(expr1, expr2) : Or(expr1, expr2);
        }
